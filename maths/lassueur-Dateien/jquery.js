...
this.options.minChars=g(this.options.minChars,e.fn.autocomplete.defaults.minChars,{min:1});
...
i.keydown(function(k){h.lastKeyPressed_=k.keyCode;
...
if(h.options.preventDefaultTab){k.preventDefault();
return false
}}break;
case 13:if(h.active_){h.selectCurrent();
if(h.options.preventDefaultReturn){k.preventDefault();
return false
}}break;
case 27:if(h.active_){k.preventDefault();
h.deactivate(true);
return false
}break;
default:h.activate()
}});
i.blur(function(){if(h.finishOnBlur_){h.finishTimeout_=setTimeout(function(){h.deactivate(true)
},200)
}})
};
e.Autocompleter.prototype.position=function(){var h=this.dom.$elem.offset();
this.dom.$results.css({top:h.top+this.dom.$elem.outerHeight(),left:h.left})
};
...
e.Autocompleter.prototype.activate=function(){var h=this;
...
e.Autocompleter.prototype.filterResult=function(h,j){if(!h.value){return false
...
e.Autocompleter.prototype.beforeUseConverter=function(k,i,h){k=this.getValue();
var j=this.options.beforeUseConverter;
if(e.isFunction(j)){k=j(k,i,h)
}return k
};
...
var j=e("<li>"+this.showResult(h.value,h.data)+"</li>");
...
e.Autocompleter.prototype.getCaret=function(){var l=this.dom.$elem;
if(e.browser.msie){var k=document.selection;
if(l[0].tagName.toLowerCase()!="textarea"){var n=l.val();
var h=k.createRange().duplicate();
h.moveEnd("character",n.length);
var j=(h.text==""?n.length:n.lastIndexOf(h.text));
h=k.createRange().duplicate();
h.moveStart("character",-n.length);
var m=h.text.length
}else{var h=k.createRange();
var i=h.duplicate();
i.moveToElementText(l[0]);
i.setEndPoint("EndToEnd",h);
var j=i.text.length-h.text.length;
var m=j+h.text.length
}}else{var j=l[0].selectionStart;
var m=l[0].selectionEnd
}return{start:j,end:m}
};
...
e.Autocompleter.prototype.getValue=function(){var i=this.dom.$elem.val();
if(this.options.useDelimiter){var h=this.getDelimiterOffsets();
return i.substring(h.start,h.end).trim()
}else{return i
}};
